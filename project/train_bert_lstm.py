...
from models.bert_lstm import Net
...
parser.add_argument("--logdir", type=str, default="checkpoints/feature_bert")
...
def eval(model, iterator, f):
    ...
    ## calc metric
    y_true =  np.array([tag2idx[line.split()[1]] for line in open("temp_bert", 'r').read().splitlines() if len(line) > 0])
    y_pred =  np.array([tag2idx[line.split()[2]] for line in open("temp_bert", 'r').read().splitlines() if len(line) > 0])
