...
char_to_id = char_mapping(train_sentences)
...
model.fit([char_sequence_train, word_sequence_train], tags_train,
           validation_data=([char_sequence_valid, word_sequence_valid], tags_valid),
            batch_size=10, epochs=10, callbacks=callbacks_list)

pred_tag = model.predict([char_sequence_valid, word_sequence_valid])
pred_tag = np.argmax(pred_tag, axis=-1)
true_tag = np.argmax(tags_valid, -1)
true_tag = [[id_to_tag[i] for i in row] for row in true_tag]
true_tag = [[x for x in row if x!='<UNK>'] for row in true_tag]

pred_tag = [[id_to_tag[i] for i in row] for row in pred_tag]
pred_tag = [[x.replace("PADword", "O") for x in pred_tag[index]][: len(true_tag[index])] for index in range(len(true_tag))]

print('LSTM_CNN_CRF Classification Report\n', metrics.flat_classification_report(pred_tag, true_tag))
