...
from models.bert_lstm_crf import Net
...
parser.add_argument("--logdir", type=str, default="checkpoints/feature_bert_crf")
...
def eval(model, iterator, f, device):
    ...
    with open("temp_bert_crf", 'w', encoding='utf-8') as fout:
    ...
    ## calc metric
    y_true =  np.array([tag2idx[line.split()[1]] for line in open("temp_bert_crf", 'r', encoding='utf-8').read().splitlines() if len(line) > 0])
    y_pred =  np.array([tag2idx[line.split()[2]] for line in open("temp_bert_crf", 'r', encoding='utf-8').read().splitlines() if len(line) > 0])
    ...
    with open(final, 'w', encoding='utf-8') as fout:
        result = open("temp_bert_crf", "r", encoding='utf-8').read()
        fout.write(f"{result}\n")

        fout.write(f"precision={precision}\n")
        fout.write(f"recall={recall}\n")
        fout.write(f"f1={f1}\n")
    os.remove("temp_bert_crf")
