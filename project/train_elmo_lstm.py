...
from models.elmo_lstm import Net
...
parser.add_argument("--logdir", type=str, default="checkpoints/feature_elmo")
...
def eval(model, iterator, f):
    ...
    with open("temp_elmo", 'w') as fout:

        for words, is_heads, tags, y_hat in zip(Words, Is_heads, Tags, Y_hat):
            y_hat = [hat for head, hat in zip(is_heads, y_hat) if head == 1]
            preds = [idx2tag[hat] for hat in y_hat]
            assert len(preds)==len(words.split())==len(tags.split())
            for w, t, p in zip(words.split()[0:], tags.split()[0:], preds[0:]):
                fout.write(f"{w} {t} {p}\n")
            fout.write("\n")


    ## calc metric
    y_true =  np.array([tag2idx[line.split()[1]] for line in open("temp_elmo", 'r').read().splitlines() if len(line) > 0])
    y_pred =  np.array([tag2idx[line.split()[2]] for line in open("temp_elmo", 'r').read().splitlines() if len(line) > 0])
