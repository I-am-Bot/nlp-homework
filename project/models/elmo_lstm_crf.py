'''Modified based on https://github.com/HandsomeCao/Bert-BiLSTM-CRF-pytorch/blob/master/crf.py'''
import torch
...
class Net(nn.Module):
    ...
    def __init__(self, tag_to_ix, hidden_dim=1024, device='cpu', finetuning=False):
        super(Net, self).__init__()
        self.tag_to_ix = tag_to_ix
        self.tagset_size = len(tag_to_ix)
        # self.hidden = self.init_hidden()
        self.lstm = nn.LSTM(bidirectional=True, num_layers=2, input_size=1024, hidden_size=hidden_dim//2, batch_first=True)
    ...
    def _score_sentence(self, feats, label_ids):
        ...
        # the 0th node is start_label->start_wordthe probability of them=1. so t begin with 1.
        for t in range(1, T):
            score = score + \
